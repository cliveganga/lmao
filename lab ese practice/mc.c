...
#define MAX_SIZE 100
void print_optimal(int s[MAX_SIZE][MAX_SIZE], int i, int j, char name)
{
    if(i ==j){
    printf("%c", name++);
    }
        ...
        printf("(");
        print_optimal(s, i, s[i][j], name);
        print_optimal(s, s[i][j]+1, j, name+s[i][j]-i+1);
        printf(")");
    }
}
...
void main() {
    ...
    int matrix_sizes[num_matrices+1];
